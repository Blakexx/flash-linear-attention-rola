...
def deprecate_kwarg(
    ...
    By default, the decorator notifies the user about the deprecated argument while the `fla_rola.__version__` < specified `version`
    ...
    Args:
        ...
        warn_if_greater_or_equal_version (`bool`, *optional*, defaults to `False`):
            Whether to show warning if current `fla_rola` version is greater or equal to the deprecated version.
        raise_if_greater_or_equal_version (`bool`, *optional*, defaults to `False`):
            Whether to raise `ValueError` if current `fla_rola` version is greater or equal to the deprecated version.
