...
fla_rola.layers.RoLA (which already returns the FLA 3-tuple, so the block calls it directly — no
adapter wrapper). Training + loglikelihood eval."""
...
from fla_rola.models.utils import Cache, FLAGenerationMixin
from fla_rola.modules import FusedCrossEntropyLoss, FusedLinearCrossEntropyLoss, GatedMLP, RMSNorm
from fla_rola.modules.l2warp import l2_warp
...
try:
    ...
    from fla_rola.models.modeling_layers import GradientCheckpointingLayer
...
class RoLABlock(GradientCheckpointingLayer):
    ...
    def __init__(self, config: RoLAConfig, layer_idx: int):
        ...
        self.attn = RoLA(hidden_size=config.hidden_size, layer_idx=layer_idx,
                         ...
                         router_bias=getattr(config, "router_bias", False), **kw)
...
class RoLAModel(RoLAPreTrainedModel):
    ...
    def __init__(self, config: RoLAConfig):
        ...
        self.layers = nn.ModuleList([RoLABlock(config, layer_idx) for layer_idx in range(config.num_hidden_layers)])
    ...
    def forward(
        ...
        **kwargs: Unpack[dict],
        ...
        if output_attentions:
            warnings.warn("`RoLAModel` does not `output_attentions` now, setting it to `False`.")
...
class RoLAForCausalLM(RoLAPreTrainedModel, FLAGenerationMixin):
    ...
    # Untied (tie_word_embeddings=False) -> no tied weights. Empty DICT, not a list: transformers
    # 5.x iterates this as a mapping (the legacy list form breaks safetensors save).
    _tied_weights_keys = {}
        ...
        try:
            ...
            if 'past_key_values' in str(exception):
                raise AttributeError(
                    f"You tried to call `generate` with a decoding strategy that manipulates "
                    f"`past_key_values`, which is not supported for {self.__class__.__name__}. "
